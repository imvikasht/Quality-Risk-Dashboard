...
export async function getDashboardTrends(req: Request, res: Response) {
  try {
    ...
    const trendsMap = new Map<string, { month: string; incr: number; lbe: number }>();
    
    for (const record of records) {
      if (!record.issuedDate) continue;
      
      const month = dayjs(record.issuedDate).format('YYYY-MM');
  ...
  try {
    ...
    for (const record of records) {
      if (record.riskLevel in distribution) {
        distribution[record.riskLevel as keyof typeof distribution]++;
      }
    }
...
export async function getProjects(req: Request, res: Response) {
  try {
    ...
    for (const r of records) {
      ...
      p.totalRecords++;
      if (r.status === 'Open') p.openRecords++;
      if (r.riskLevel === 'Critical') p.criticalRecords++;
    }
