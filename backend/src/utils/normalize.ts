 ...
 */
export function normalizeStatus(value: unknown): 'Open' | 'Closed' | 'Withdrawn' {
  if (value === null || value === undefined) return 'Open';
  const str = String(value).trim().toLowerCase();
  return (STATUS_MAP[str] as 'Open' | 'Closed' | 'Withdrawn') ?? 'Open';
}
 ...
 */
export function normalizeProject(value: unknown, fallback: string): string {
  if (value === null || value === undefined) return fallback;
  const str = String(value).trim();
  if (str === '') return fallback;

  const lower = str.toLowerCase();
  if (lower === 'dorra') return 'Dorra';
  if (lower === 'crpo160' || lower === 'crpo-160') return 'CRPO-160';
  return str;
}
 ...
 */
export function normalizeLocation(value: unknown, fallback: string): string {
  if (value === null || value === undefined) return fallback;
  const str = String(value).trim();
  if (str === '') return fallback;
 ...
 */
export function normalizeDiscipline(value: unknown): string | undefined {
  if (value === null || value === undefined) return undefined;
  const str = String(value).trim();
